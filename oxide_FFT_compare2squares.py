...
img_name = '2a_0.75_5x5_hs'  # Take a file name from the list, or insert your file name here
file_name = img_name + '.png'



run_no = 10  #  appends a number to image, useful tool for when you don't want to overwrite old data
image_size = 7  # provide the image size in microns
...
square_1_origin = [150,388] # 55,60-470 increments of 82,  142 224 306 388
square_2_origin = [100,0] # 55,0 size:50
square_size = 82 # size of square in pixels
...
if nudes == 1:
    image_loc = 'Oxide Data/Nudes/' + file_name
else:
    image_loc = 'Oxide Data/' + file_name
...
F2 = fftpack.fftshift(F1)
cm_2d = np.abs(F2) ** 2
s1_sav_loc = 'res/oxide/comparesquares/' + img_name + '_square1_' + str(run_no) + '.png'
py.imsave(s1_sav_loc, np.log(cm_2d), dpi=300, cmap='rainbow')
...
psd1D_1 = SpectralFFT(crop_square1_array)
...
horz_1 = np.linspace(0, np.sqrt((square_size / 2) ** 2 + (square_size / 2) ** 2),
                   len(psd1D_1)) / (image_size * ratio * 1000)
...
py.semilogy(horz_1[1:region1:1], psd1D_1[1:region1:1], 'ko', label='FFT Region 1', ms=5)
...
fig_sav_loc = 'res/oxide/comparesquares/' + img_name + 'plot_' + str(run_no) + '.svg'
py.savefig(fig_sav_loc, dpi=300)
fig_sav_loc = 'res/oxide/comparesquares/' + img_name + 'plot_' + str(run_no) + '.png'
py.savefig(fig_sav_loc, dpi=300)
py.show()
